...
def send_knx_request(
    ...
    # <- Receiving the connexion state response
    logger.debug('waiting for a connection response (CONNECTION_RESPONSE -> conn_resp)')
    data_recv2, addr = sock.recvfrom(buf_size)
    con_stat_resp_object = knxnet.decode_frame(data_recv2)
    ...
    logger.debug('>>> TUNNELING_REQUEST:\n{}'.format(tunn_ack_object))
    sock.sendto(
    tunn_ack_object.frame,
    (gateway_ip, gateway_port)
     )
    # <- Receiving tunneling ack 
    data_rec3,addr = sock.recvfrom(buf_size)
    tunneling_ack_object = knxnet.decode_frame(data_rec3)
    logger.info('reponse status: {}'.format(tunneling_ack_object.status))
    logger.info('reponse status: {}'.format(tunneling_ack_object.sequence_counter))
    # -> sending disconnect _request
    disc_req_objt = knxnet.create_frame(
    conn_channel_id,
    control_endpoint
    )
    logger.debug('>>> DISCONNECT_REQUEST:\n{}'.format(disc_req_objt))
    sock.sendto(
    disc_req_objt.frame,
    (gateway_ip, gateway_port)
    )
